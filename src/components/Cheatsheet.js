  ...
  return (
    <section className={`border-l transition-all duration-700 ${isExpanded ? 'lg:min-w-[750px] lg:max-w-[750px]' : 'lg:max-w-[400px]'}  relative`}>
        {/*<ToolTip tooltip={isExpanded ? 'Collapse' : 'Extend'}>*/}
            <button onClick={() => setIsExpanded(!isExpanded)} className='invisible lg:visible text-lg border rounded shadow-md text-indigo-900 p-4 absolute top-0 left-0 bg-white hover:bg-indigo-50 transition-colors -translate-x-1/2 z-10'>
                {isExpanded ? <BsChevronCompactRight/> : <BsChevronCompactLeft/>}
            </button>
        {/*</ToolTip>*/}

        <div className={`px-3 lg:px-12 py-4 lg:py-10 lg:h-[1000px] transition-all overflow-y-auto ${!isExpanded && 'blur-sm hover:blur-none'} scrollbar scrollbar-thin scrollbar-thumb-orange-300 scrollbar-track-indigo-50 w-full`}>
            <header>
