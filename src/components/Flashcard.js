...
function Flashcard({ question, isCorrect, cardImage, example }) {
  ...
  // Find the General American English voice
  const generalAmericanVoice = voices.find((voice) => voice.name === "Google US English");

  const setGeneralAmericanVoice = (e) => {
    e.stopPropagation(); // Prevent the event from bubbling up
    if (generalAmericanVoice) {
      speak({ text: isCorrect, voice: generalAmericanVoice });
    }
  };
  ...
  return (
    <div className={isFlipped ? " flex-grow flip-card flipped min-w-[250px] min-h-[300px] cursor-pointer" : "flex-grow flip-card min-w-[250px] min-h-[250px] cursor-pointer"} onClick={flipCard}>
      <div className="flip-card-inner shadow-md rounded-md border">
        ...
        <div className="flip-card-back flex flex-col items-center">
          ...
          <div className="basis-2/3 p-2 w-full flex flex-col justify-around items-start text-left">
            <div className="flex items-center cursor-pointer hover:opacity-50 transition-all" onClick={setGeneralAmericanVoice}>
