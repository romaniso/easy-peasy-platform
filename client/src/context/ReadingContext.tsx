...
const useDictionary = (): IDictionaryContext => {
  ...
  const addWordToDictionary = (newSelectedWord: IDictionaryUnit): ToastType => {
    const isWordAlreadyAdded = selectedWords.some(
      (word) => word.word === newSelectedWord.word
    );
  ...
  const removeWordFromDictionary = (wordId: string): void => {
    const updatedSelectedWords: IDictionaryUnit[] = selectedWords.filter(
      (word) => word.id !== wordId
    );
  ...
  const editWordInDictionary = (wordId: string, newValue: string): void => {
    const { word, id, audio } = selectedWords.find(
      (word) => word.id === wordId
    ) as IDictionaryUnit;
    const updatedSelectedWords: IDictionaryUnit[] = selectedWords.map(
      (selectedWord) => {
        if (selectedWord.id === wordId)
          return { word, id, audio, definition: newValue };
...
export const ReadingContextProvider = ({
  children,
}: ReadingContextProviderProps): JSX.Element => (
...
export const useSelectedWords = () =>
  useContext(ReadingContext as React.Context<IDictionaryContext>).selectedWords;
export const useIsHeadingsOpened = () =>
  useContext(ReadingContext as React.Context<IDictionaryContext>)
    .isHeadingsOpened;
...
export const useRemoveWordFromDictionary = () =>
  useContext(ReadingContext as React.Context<IDictionaryContext>)
    .removeWordFromDictionary;
export const useEditWordInDictionary = () =>
  useContext(ReadingContext as React.Context<IDictionaryContext>)
    .editWordInDictionary;
export const useOpenHeadings = () =>
  useContext(ReadingContext as React.Context<IDictionaryContext>).openHeadings;
