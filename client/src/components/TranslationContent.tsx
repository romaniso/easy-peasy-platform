    ...
    <span className="text-indigo-50">
      <span className="flex justify-between flex-wrap">
        <dt className="font-bold">
          {word}
          {fetchedData && (
            <span className="opacity-80 ml-2 font-light">
              {fetchedData.transcription}
            </span>
          )}
        </dt>
      ...
      {fetchedData && (
        <span>
          {fetchedData.definitions.map((definitionsArr, arrIndex) => {
            return (
              <span className="block border-white/50 py-2" key={arrIndex}>
                {definitionsArr.map((definition, defIndex) => (
                  <dd className="block mb-2" key={defIndex}>
                    {definition}
                  </dd>
                ))}
              </span>
