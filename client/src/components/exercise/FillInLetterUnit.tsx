...
const FillInLetterUnit: React.FC<FillInLetterUnitProps> = ({wordIndex, word, onFill, coveredIndexes, results}) => {
    const [activeInputIndex, setActiveInputIndex] = useState<number | null>(null);
    const [wordToComplete, setWord] = useState<string[]>(word);
    const inputRefs = useRef<(HTMLInputElement | null)[]>([]);

    const handleChange = ({target} : React.ChangeEvent<HTMLInputElement>, charIndex: number): void => {
        const {value} = target;
    ...
    const handleOnKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, inputIndex: number): void => {
        if (e.key === "Backspace") {
            e.preventDefault();
        ...
        <div className="inline-flex items-center gap-1 mb-6 md:mb-8">
            ...
            {word.map((char: string, charIndex: number) => {
                const inputClasses = className(
                    ...
                    {
                        "!bg-red-100 dark:!bg-red-500/30": results && Array.from(results[wordIndex] as number[]).includes(charIndex),
                        "!bg-green-100 !bg-green-500/30": results && !(Array.from(results[wordIndex] as number[]).includes(charIndex)),
                    }
                );
