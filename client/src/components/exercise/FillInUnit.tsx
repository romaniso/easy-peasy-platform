...
export const FillInUnit = ({
  unit,
  index,
  onChange,
}: FillInUnitProps): JSX.Element => {
  return (
    <span className="-mt-0.5">
      {unit.split("***").map((part, partIndex) => {
        let renderedPart: React.ReactElement = (
          <span key={"renderedPart-" + index}>{part}</span>
        );
        const HINT_DIVIDER = "(";
        if (part.includes(HINT_DIVIDER)) {
          const partArr: string[] = part.split(HINT_DIVIDER);
          renderedPart = (
            <span key={"renderedPart-" + index}>
              {partArr[0]}
              <span className="italic ml-1 text-orange-500 font-thin leading-8">
                {HINT_DIVIDER}
                {partArr[1]}
              </span>
            </span>
        ...
        return partIndex === 1 ? inputPart : renderedPart;
      })}
    </span>
  );
};
