...
const ExerciseFillInLetter: React.FC<ExerciseFillInLetterProps> = ({ questions, onChange, results }) => {
    ...
    const handleWord = (word: string, index: number): void => {
        ...
        const arrCoveredIndexes: number[] = Array.from(coveredLetterIndexesForUnit).sort((a, b) => a - b);
    ...
    const renderedExercise = insertedWords.map((word, wordIndex) => {
        ...
        return <div className='flex items-baseline gap-2' key={wordIndex}>
            <FillInLetterUnit
                key={wordIndex}
                word={word}
                wordIndex={wordIndex}
