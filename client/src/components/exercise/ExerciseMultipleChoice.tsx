    ...
    <ul>
      {questions.map(({ question, options }, index) => {
        ...
        const renderedSentence = (
          <div
            className="text-indigo-900 dark:text-indigo-200 text-xl mb-4"
            key={`sentence-${index}`}
          >
            ...
            {question.split("***").map((part, partIndex) => {
              ...
              return partIndex === 1 ? (
                ...
                <span key={`part-${index}-${partIndex}`}>{part}</span>
              );
            })}
          </div>
        );
        ...
        const renderedChoices = (
          <ul
            className="mb-8 md:ml-8 text-base md:text-lg text-orange-600 dark:text-indigo-200 md:grid md:grid-cols-4 md:gap-8"
            key={`choices-${index}`}
          >
            {options?.map((option, optionIndex) => {
              ...
              return (
                <li
                  key={`choice-${index}-${optionIndex}`}
                  className="flex gap-2 m-2 md:m-0"
                >
                  {choiceLetter}){" "}
                  <span
