...
const CheckboxButton: React.FC<RadioButtonProps> = ({item, small}) => {
    return (
        <label
            key={item.text}
            htmlFor={item.text}
        ...
        >
            <input type="checkbox" id={item.text} name={item.text} value={item.text} className='absolute inset-0 w-full h-full inline-block appearance-none rounded-xl outline-0 transition-colors cursor-pointer peer'/>
