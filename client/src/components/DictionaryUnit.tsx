...
import {useEditWordInDictionary, useRemoveWordFromDictionary} from "../context/ReadingContext";
...
const DictionaryUnit: React.FC<IDictionaryUnit> = ({word, definition, audio, id}) => {
    ...
    const editionArea: React.ReactElement =
        <form className='basis-full relative' onSubmit={(e: React.FormEvent) => handleEdit(e, id, editedValue)}>
            <textarea id={id} name='editedDefinition' autoFocus defaultValue={definition} value={editedValue} className='resize-none w-full min-h-[80px] leading-5 border outline-0 focus:border-orange-500 focus:dark:border-orange-500 text-sm text-indigo-600 dark:text-indigo-200 dark:border-gray-500 px-2 py-0.5 dark:bg-stone-900 rounded-md scrollbar scrollbar-thin scrollbar-thumb-orange-200 scrollbar-track-orange-400' onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setEditedValue(e.target.value)}/>
    ...
    return <div className='flex justify-between items-start gap-2 [&:not(:last-of-type)]:border-b dark:border-gray-500 py-3'>
        ...
        <div className='translate-y-0.5 flex border-l border-indigo-200 pl-2'>
            <ToolTip tooltip='Edit'>
                <button onClick={() => setIsBeingEdited(!isBeingEdited)}><RiEditFill  className='text-xl text-indigo-900 dark:text-indigo-200 hover:scale-125 transition-transform'/></button>
            </ToolTip>
            <ToolTip tooltip='Remove'>
                <button onClick={() => removeWord(id)}><RiDeleteBinFill  className='text-xl text-red-600 dark:text-red-400 hover:scale-125 transition-transform'/></button>
            </ToolTip>
        </div>
