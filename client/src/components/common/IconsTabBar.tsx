...
const IconsTabBar: React.FC<IconTabBarProps> = ({ items, activeTab, setActiveTab, className}) => {
    ...
    const renderedTabs = items.map((item, index) => {
        return (
            <div className={`
                basis-1/${items.length} flex-grow flex flex-col justify-between items-center relative text-center after:[content: ""] before:[content: ""] after:transition-colors after:duration-300 before:transition-colors before:duration-300
